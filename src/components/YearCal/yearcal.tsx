...
const YearCal: React.FC<YearViewProps> = ({
    currentDate,
    onSelectDate,
    onSelectMonth,
}) => {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 p-2 h-full w-full">
                {monthsOfYear.map((month) => {
                    ...
                    const nextMonthDays = Array.from(
                        {
                            length:
                                42 - (daysInMonth.length + prevMonthDays.length),
                        },
                            ...
                            <div className="grid grid-cols-7 border rounded text-center text-[9px] hover:cursor-pointer">
                                {allDays.map((day, index) => {
                                    ...
                                    return (
                                        <div
                                            key={day.toISOString()}
                                            className={`p-2 border-b border-r transition ease-in-out ${isLastColumn ? "border-r-0" : ""
                                                } ${isLastRow ? "border-b-0" : ""} ${isTopLeftCorner ? "rounded-tl" : ""
                                                }
                        ${isTopRightCorner ? "rounded-tr" : ""}
                        ${isBottomLeftCorner ? "rounded-bl" : ""}
                        ${isBottomRightCorner ? "rounded-br" : ""} ${isToday(day) && isCurrentMonth
