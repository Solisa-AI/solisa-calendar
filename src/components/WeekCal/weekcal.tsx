...
const WeekCal: React.FC<WeeklyCalendarProps> = ({ events }) => {
  ...
  const formatMonthRange = () => {
    ...
    if (firstDay.getMonth() === lastDay.getMonth()) {
      ...
      return `${firstDay.toLocaleString("default", { month: "short" })} - ${lastDay.toLocaleString("default", { month: "long" })} ${lastDay.getFullYear()}`
    }
  }
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {/* Calendar Grid */}
        <div className="grid grid-cols-8 h-[800px] overflow-y-auto">
          {/* Time Column */}
          <div className="border-r pt-20 sticky left-0 bg-white z-10">
            {hours.map((hour) => (
              <div key={hour} className="h-10 border-t flex items-start justify-end pr-2 text-sm text-gray-500">
                {formatTime(hour)}
              </div>
            ))}
          </div>
          ...
          {weekDays.map((day, index) => (
            <div key={index} className="relative border-r">
              {/* Day Header - Increased height */}
              <div
                className={cn(
                  "h-20 border-b p-1 text-center sticky top-0 bg-white flex flex-col justify-center z-20",
                  new Date().toDateString() === day.toDateString() ? "bg-blue-50" : "",
                )}
              >
                <div className="text-sm font-medium mb-1">{day.toLocaleString("default", { weekday: "short" })}</div>
                <div
                  className={cn(
                    "text-lg inline-flex items-center justify-center w-8 h-8 rounded-full mx-auto",
                    new Date().toDateString() === day.toDateString() ? "bg-blue-500 text-white" : "",
                  )}
                >
                  {day.getDate()}
                </div>
              ...
              {/* All-day events */}
              {processedEvents[formatDate(day)]?.allDay.map((event, eventIndex) => (
                <div
                  key={event.id}
